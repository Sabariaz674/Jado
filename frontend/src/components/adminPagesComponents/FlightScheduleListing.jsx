...
const FlightScheduleListing = ({ onDeleteFlight, onUpdateFlight }) => {
  ...
  useEffect(() => {
    // Fetch flights after certain operations or when the component loads
  ...
  // Toggle visibility of more flights
  const handleToggle = () => {
    setVisibleCount(prev => (prev < flights.length ? flights.length : 3)); // Show all or reset to 3
    if (visibleCount >= flights.length) window.scrollTo({ top: 0, behavior: 'smooth' }); // Scroll to top when toggling
  };
  ...
  if (status === 'loading') {
    return <div className="flex justify-center items-center h-full"><p className="text-xl text-gray-500">Loading flights...</p></div>;
  }

  if (status === 'failed') {
    return <div className="flex justify-center items-center h-full"><p className="text-xl text-red-500">Failed to fetch flights. Please try again later.</p></div>;
  }
